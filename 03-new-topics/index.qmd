---
title: New topics in network modeling
subtitle: 2nd Chilean Summer School about Social Network Research
author: George G. Vega Yon, Ph.D.
institute:
  - The University of Utah
format: revealjs
bibliography: ../references.bib
date: 2026-01-16
slide-number: true
html-math-method: katex
embed-resources: true
footer: New topics in network modeling -- ggvy.cl -- george.vegayon@utah.edu
incremental: true
---

## Overview

\newcommand{\tpose}[1]{#1^{\mathbf{t}}}
\newcommand{\Prergm}[0]{P_{\mathcal{Y}, \bm{\theta}}}
\newcommand{\Y}[0]{\bm{Y}}
\newcommand{\y}[0]{\bm{y}}
\newcommand{\thetas}[0]{\bm{\theta}}
\newcommand{\isone}[1]{\mathbb{1}\left(#1\right)}
\renewcommand{\Pr}[1]{\mathrm{P}\left(#1\right)}

- With **more data** and **computing resources**, the things that we can ask and do with networks are becoming increasingly (even more) exciting and complex.

- In this section, I will introduce **some** of the latest advancements and forthcoming topics in network modeling.

# Part I: New models and extensions {background-color="black"}

## Multi-ERGMs

::: {.columns}

::: {.column}
- In @krivitskyTaleTwoDatasets2023a, the authors present a start-to-finish pooled ERGM example featuring heterogeneous data sources.

- They **increase power** and allow **exploring heterogeneous effects** across types/classes of networks.
:::

::: {.column}
![](networks.png){width=100%}
:::

:::

## Statistical power of SOAM

::: {style="font-size: 80%"}
- Stochatsic Actor Oriented Models [SOAM] (or SIENA models) are used for the **co-evolution of behavior and network structure**.

- The discussion around statistical power in Social Network Studies is relatively new.

- **SOAM** @stadtfeldStatisticalPowerLongitudinal2020 proposes ways to perform power analysis for Siena models. At the center of their six-step approach is simulation.
:::

![](siena-power-analysis.png)

## Bayesian ALAAM

Ever wondered how to model influence exclusively?

::: {style="font-size:60%"}
$$
\Pr{\text{Sufficient Stats} | Features} \text{ vs } \Pr{Features | \text{Sufficient Stats}}
$$
:::

::: {.columns style="font-size: 0.8em;"}
::: {.column}
- The Auto-Logistic Actor Attribute Model [ALAAM] is a model that allows us to do just that.

- ALAAMs switch the focus from network structure to behavior influence by the network:

- @koskinenBayesianAnalysisSocial2022 extends the ALAAM model to a Bayesian framework.
:::

::: {.column}
- It provides greater flexibility to accommodate more complicated models and add extensions such as hierarchical models.
![Figure 1 reproduced from @stivalaUsingSampledNetwork2020](alaam-diagram.png)
:::

:::

## Relational Event Models

- REMs are great for modeling (timed) sequences of ties (instead of panel or cross-sectional).

- Examples include: email exchanges, conflict events, and social interactions.

-  @buttsRelationalEventModels2023 provides a general overview of Relational Event Models [REMs,] new methods, and future steps.

![Figure 3 reproduced from @brandenbergerInterdependenciesConflictDynamics2020](rem.png)

## Big ERGMs

::: {.columns}

::: {.column width=45%}
- **ERGMs** In @stivalaExponentialRandomGraph2020a, a new method is proposed to estimate large ERGMs (featuring millions of nodes).
:::

<!-- [^nonet]

[^nonet]: No network visualization in the paper... as it usually happens in SNA papers! -->

::: {.column width=45%}
![Partial map of the Internet based on the January 15, 2005 data found on opte.org. -- [Wiki](https://en.wikipedia.org/w/index.php?title=File:Internet_map_1024.jpg&oldid=1061830697)](bignet.jpg){width=100%}
:::

:::

## Exponential Random *Network* Models

- Wang, Fellows, and Handcock recently published a re-introduction of the ERNM framework [@wangUnderstandingNetworksExponentialfamily2023].

- ERNMs generalize ERGMs to incorporate behavior and are the cross-sectional cousin of SIENA models.

\begin{align*}
\text{ER\textbf{G}M}: & \Prergm(\bm{Y}=\bm{y} | \bm{X}=\bm{x}) \\
\text{ER\textbf{N}M}: & \Prergm(\bm{Y}=\bm{y}, \bm{X}=\bm{x})
\end{align*}

## Other new (and not so new) models and extensions



# Part II: Shameless self-promotion {background-color="black"}

## ERGMitos: Small ERGMs

- **ERGMitos**[^ito] [@vegayonExponentialRandomGraph2021] leverage small network sizes to use exact statistics.

[^ito]: From the Spanish suffix meaning small.


![Five small networks from the `ergmito` R package](littlenet.jpg){width=40%}

## Discrete Exponential-family Models {style="font-size: 0.8em;"}

::: {.columns}

::: {.column width=40%}
- ERGMs are a particular case of Random Markov fields.

- We can use the ERGM framework for modeling vectors of binary outcomes, *e.g.*, the consumption of \{tobacco, MJ, alcohol\}.

- Moreover, similar to TERGMs (temporal ERGMs), we can model changes in these vectors over time.
::: 

::: {.column width=55%}
![](defm.png)
:::
:::

You can learn more in the project's website <https://uofuepibio.github.io/defm/>



## Power analysis in ERGMs

::: {style="font-size: 80%"}
- Using conditional ERGMs, we can do power analysis for network samples [@vegayonPowerMulticollinearitySmall2023].

- With multi-network studies becoming increasingly common, the question of how many networks we need is now more important (for doing science *and* funding science).
:::

![Reproduced from @krivitskyRejoinderDiscussionTale2023](fig1-rejoinder.png)

## Imaginary Network Motifs {.scrollable}

::: {style="font-size:60%"}
- Cognitive Social Structures [CSS] refers to individuals' perception of social networks (usually their own).

- In CSS studies, people are asked about different types of ties in the form of "Does $i$ communicates/asks for advice/is friend of/etc $j$".
:::

![](tanaka-css-by-id1.jpg){width=100%}

## Imaginary Network Motifs (cont.) {.scrollable}

::: {style="font-size: 60%"}
> [I]individuals completely conceive or drop reciprocal structures to maintain a balanced structure in their perceptions -- @tanakaImaginaryNetworkMotifs2024

![Reproduced from @tanakaImaginaryNetworkMotifs2024](dyad-census-imaginary.jpg)
:::

## Two-step estimation ERGMs

::: {style="font-size: 0.7em;"}

- Conditioning the ERGM on an observed statistic "drops" the associated coefficient.

- Hypothesis: As $n$ increases, conditional ERGM estimates are consistent with the full model:

:::

![Simulation study trying to demonstrate the concept (Work in progress)](condergm.png)

## Thanks!

## Bonus track: Why network scientists don't use ERGMs?

> Attempts to overcome these problems by extending the blockmodel have focused particularly on the use of (more complicated) p∗ or exponential random graph models, but **while these are conceptually appealing**, they quickly **lose the analytic tractability** of the original blockmodel **as their complexity increases**. 
>
> -- @karrerStochasticBlockmodelsCommunity2011


## References
